...
if (!isset($_GET['id_tarea']) || !is_numeric($_GET['id_tarea'])) {
    header('Location: tareas_user.php?error=ID de tarea no especificado.');
    exit;
}
...
if (!$nombre_t) {
    header('Location: tareas_userphp?error=Tarea no encontrada.');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $update_query = "UPDATE tareas SET progreso = ?";
    $stmt = $mysqli->prepare($update_query);
    $stmt->bind_param('i', $progreso);

    if ($stmt->execute()) {
        header('Location: tareas_user.php?id_proyecto=' . $id_proyecto . '&success=Tarea actualizada correctamente.');
        exit;
    } else {
    ...
    <div class="container">
        ...
        <form method="post" action="">
            ...
            <div class="buttons-container">
                ...
                <a href="tareas_user.php?id_proyecto=<?php echo htmlspecialchars($id_proyecto); ?>" class="back-button"><button type="button">Volver</button></a>
            </div>
        </form>
    </div>
</body>
